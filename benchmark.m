...
I_NYU = (readall(imageDatastore([filepath2  p2 '/*.png'])));
...
for s = 1:length(I_our)
    if length(I_our{s}) > 0
        ...
        I_NYU_VAL_tmp = double(I_NYU{s})/max(max(double(I_NYU{s})));
...
parfor g = 1:length(I_our_resize)
    ...
    I_gradient_test = {};
        for ss = 1:length(n_hs)
            ...
            I_CSPN_test_input = I_CSPN_test_input/max(max(I_CSPN_test_input));
